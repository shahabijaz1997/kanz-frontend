...
const CreateDeal = () => {
  ...
  const getDealStepDetails = async () => {
    try {
      ...
      if (status === 200) {
        ...
        for (
          let i = 0;
          i < data?.status?.data?.step_titles[event]?.length;
          i++
        ) {
        ...
        let options: any = [];
        let stepper: any;
        data?.status?.data?.steps[step - 1][event]?.sections?.sort(
          (a: any, b: any) => a.index - b.index
        );
        data?.status?.data?.steps[step - 1][event]?.sections.forEach(
          (sec: any) => {
            sec?.fields?.sort((a: any, b: any) => a.index - b.index);
            for (let i = 0; i < sec?.fields?.length; i++) {
              const field = sec?.fields[i];
              field?.options &&
                field?.options?.sort((a: any, b: any) => a.index - b.index);
            }
            let f = sec?.fields?.find((ques: any) => {
              return _dependencies?.find(
                (dep: any) => dep?.dependable_field === ques?.id
              );
            });
            // Check Unique Selling Points
            if (
              sec?.display_card &&
              sec?.fields?.some((field: any) => field?.value)
            ) {
            ...
            // Check External Links
              ...
              let sect: any = data?.status?.data?.steps[step - 1][
                event
        ...
        if (stepper) {
          let step = all_steps?.find(
            (ts: any) => ts?.text === stepper[event]?.title
          );
    ...
    try {
      ...
      if (multipleFieldsPayload?.length > 0 || deleted.length > 0) {
        ...
        fields = all_fields?.map((field: any) => {
          let selected;
          if (
            field?.field_type === Constants.MULTIPLE_CHOICE ||
            field?.field_type === Constants.DROPDOWN
          )
            selected = field?.options?.find((opt: any) => opt.selected)?.id;
          if (
            field?.field_type === Constants.NUMBER_INPUT ||
            field?.field_type === Constants.TEXT_BOX ||
            field?.field_type === Constants.TEXT_FIELD ||
            field?.field_type === Constants.URL ||
  ...
  const multipleChoice = (ques: any, secIndex: number, section: any) => {
    ...
    if (dependant) {
      let field = section?.fields?.find(
        (q: any) => q.id === dependant.dependable_field
      );
                    ...
                    <li
                      ...
                      onClick={() => {
                        ...
                        if (
                          ...
                          let section = dealData[step - 1][
                            event
                          ]?.sections.find(
                            (sec: any) => sec.index === secIndex
                          );
  ...
  const numberInputUI = (ques: any, secIndex: number, section: any) => {
    ...
    if (ques?.input_type === InputType.CURRENCY)
      ...
      placeholder = language?.v3?.common?.percent;
          ...
          file={
            ques?.value?.url
              ? {
                  ...
                  id: ques?.id,
                  fid: ques?.value?.id,
                  type: ques?.value?.type,
                  size: ques?.value?.size,
                  dimensions: ques?.value?.dimensions,
                  file: ques?.value?.file,
                }
              : {}
          }
          setModalOpen={() => {
          ...
          <section className="mb-5 w-full relative mt-1">
            <div className="relative w-full" style={{ zIndex: 101 }}>
              <Selector
                ...
                options={options}
                onChange={(v: any) => {
                  let opt = ques?.options?.find(
                    (op: any) => op.statement === v.value
                  );
                  dispatch(
                    saveDealSelection({
                      option: opt,
  ...
  const renderQuestionType = (ques: any, secIndex: number, section: any) => {
    if (
      ...
      if (ques?.field_type === Constants.MULTIPLE_CHOICE)
        ...
        return numberInputUI(ques, secIndex, section);
      else if (ques?.field_type === Constants.FILE)
        return attachmentsUI(ques, secIndex, section);
      else if (ques?.field_type === Constants.DROPDOWN)
        return dropdownsUI(ques, secIndex, section);
      else if (ques?.field_type === Constants.SWITCH)
        return switchInputUI(ques, secIndex, section);
      else if (ques?.field_type === Constants.TEXT_BOX)
        return textAreaInputUI(ques, secIndex, section);
      else if (ques?.field_type === Constants.TEXT_FIELD)
        return textFieldInputUI(ques, secIndex, section);
      else if (ques?.field_type === Constants.URL)
        return URLInput(ques, secIndex, section);
      else if (ques?.field_type === Constants.CHECK_BOX)
        return termUI(ques, secIndex);
      else if (ques?.field_type === Constants.DATE)
        return dateUI(ques, secIndex, section);
      else
        return (
          <ReviewDeal
            language={language}
            ...
            authToken={authToken}
            navigate={navigate}
            showDeal={step === totalSteps?.all.length}
          />
        );
    }
  };
    ...
    dealData[step - 1][event]?.sections.forEach((sec: any, index: number) => {
      ...
      fields.forEach((ques: any) => {
        if (ques?.field_type === Constants.SWITCH) {
          ...
          ques?.field_type === Constants.MULTIPLE_CHOICE ||
          ...
          ques?.field_type === Constants.NUMBER_INPUT ||
          ques.field_type === Constants.TEXT_BOX ||
          ques.field_type === Constants.TEXT_FIELD ||
          ques.field_type === Constants.URL ||
          ques.field_type === Constants.DATE
        ) {
          let dependantQuesion = sec?.fields?.find(
            (field: any) => field.id === ques?.dependent_id
          );
    ...
    if (dealData[step - 1]?.dependencies?.length > 0) {
      ...
      if (multipleFieldsPayload?.length > 0) {
        ...
        multipleFieldsPayload.forEach((field: any) => {
          ...
          if (
            field.field_type !== Constants.URL ||
            (field.field_type === Constants.URL && field.value)
          )
            newVals.push(true);
        });
      ...
      <aside className="w-full pt-14 px-12">
            ...
            {loading ? (
              ...
              <React.Fragment>
                ...
                !dealData[step - 1][event]?.sections.length ? (
                  ...
                  React.Children.toArray(
                    dealData[step - 1][event]?.sections.map(
                                ...
                                {section?.fields?.length > 0 &&
                                  React.Children.toArray(
                                    section?.fields?.map((ques: any) =>
                                      renderQuestionType(ques, index, section)
                                    )
                                  )}
                                ...
                                <Button
                                  ...
                                  onClick={() => {
                                    if (!section?.display_card) {
                                      const element =
                                        dealData[step - 1][event]?.sections[
                                          index
                                        ]?.fields?.at(-1);
                                      if (!isValidUrl(element.value))
                                        return toast.warning(
                                          language.promptMessages
                                            .pleaseEnterUrl,
                                          toastUtil
                                        );
                                      if (
                                        !multipleFieldsPayload?.some(
                                          (elem: any) =>
        ...
        <Drawer isOpen={open} setIsOpen={(val: boolean) => setOpen(val)}>
          ...
          <p className="text-neutral-700 font-normal text-sm text-justify">
            ...
            error officia necessitatibus dolor eius quam magni, atque
            recusandae, consequatur beatae, maiores odit. Laborum dicta animi
            maxime provident quidem assumenda praesentium nihil temporibus?
          </p>
        </Drawer>
      </aside>
    </main>
