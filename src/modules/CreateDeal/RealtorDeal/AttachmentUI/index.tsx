...
const AttachmentUI = ({ language, orientation }: any) => {
    ...
    const [payload, setPayload] = useState({ links: [1] })
    ...
    return (
        <section className="flex items-start justify-center flex-col mt-10 max-w-[420px] min-w-[400px] screen500:max-w-[300px]">
            <h3 className="text-neutral-700 font-medium text-base w-[420px]">
                <span>{language?.v3?.button?.upload_docs}</span>&nbsp;<span className="text-cc-blue font-medium cursor-pointer" onClick={() => setOpen(true)} >
            ...
            <div className="w-full mt-2 p-[18px] rounded-lg check-background border border-grey flex flex-col">
                ...
                <button onClick={() => {
                    setPayload(prev => {
                        const _links = [...prev.links, prev.links.length + 1];
                        return { ...prev, links: _links }
                    })
                }}
                    className="cursor-pointer py-[9px] px-[17px] w-full rounded-md border-[1px] border-neutral-900 inline-flex justify-center gap-2 items-center bg-white mt-3">
                    <img src={AddIcon} alt="Add" />
                    <small className="text-sm font-medium text-neutral-900">{language?.v3?.button?.add_new_link}</small>
                </button>
            </div>
        </section>
    )
