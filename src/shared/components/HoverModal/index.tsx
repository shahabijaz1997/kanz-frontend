...
const HoverModal = ({ children, width = "w-[280px]", height = "h-[180px]" }: any) => {
    const orientation: any = useSelector((state: RootState) => state.orientation.value);

    return (
        <aside className={`${width} ${height} absolute top-[-10px] transition-all ${orientation === "rtl" ? "right-[60px] left-0" : "left-[83px] right-0"}`} style={{ zIndex: 99 }}>
