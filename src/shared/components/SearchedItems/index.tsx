    ...
    const renderSegregatedItems = () => {
        ...
        return (
            React.Children.toArray(
                filtered.map((it: any) => <div onClick={()=>passItemSelected(it?.name)} className={`cursor-pointer rounded-md py-1 px-2 bg-cbc-check text-neutral-700 font-normal text-sm hover:bg-cbc-check-hover ${it.blue && "border-2 border-blue-500"}`}>{it.name}</div>)
            ))
    };
